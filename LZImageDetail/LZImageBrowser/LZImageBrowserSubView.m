...
- (void)initView {
    ...
    //长按等待时间
    longPressGest.minimumPressDuration = 0.4;
    //长按时候,手指头可以移动的距离
...
//长按手势
-(void)longPressView:(UITapGestureRecognizer *)doubleTap
{
    if ([self.delegate respondsToSelector:@selector(SQ_imageBrowserSubViewLongPressWithModel:)]) {
        [self.delegate SQ_imageBrowserSubViewLongPressWithModel:_imageBrowserModel];
    }
...
- (UIScrollView *)subScrollView {
    if (_subScrollView == nil) {
        ...
        _subScrollView.minimumZoomScale = 0.8;//最小缩小倍数

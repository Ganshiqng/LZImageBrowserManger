...
- (void)initView {
    ...
    self.backgroundColor = [[UIColor lightGrayColor] colorWithAlphaComponent:.7];
...
//长按
-(void)SQ_imageBrowserSubViewLongPressWithModel:(LZImageBrowserModel *)imageBrowserModel
{
    if ([self.delegate respondsToSelector:@selector(SQ_imageBrowserMianViewLongPressWithModel:)]) {
        [self.delegate SQ_imageBrowserMianViewLongPressWithModel:imageBrowserModel];
    }
}
//改颜色
